...
export default function PlanSelector({ hasTrialed }: { hasTrialed: boolean }) {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4 py-16">
      ...
      <div className="text-center mb-10">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mb-2">
          {hasTrialed ? 'プランを再選択してください' : 'プランを選択してください'}
        </h1>
        <p className="text-gray-500 text-sm max-w-sm mx-auto">
          {hasTrialed
            ? 'ご利用を再開するプランを選択してください。'
            : <>どのプランも<span className="font-semibold text-gray-700">14日間無料</span>でお試しいただけます。トライアル終了後は選択したプランで自動的に課金が始まります。</>
          }
        </p>
      </div>
      ...
      {/* 案内バナー */}
      {hasTrialed ? (
        <div className="w-full max-w-3xl mb-8 bg-yellow-50 border border-yellow-200 rounded-xl px-5 py-4 text-sm text-yellow-800">
          <p className="font-semibold mb-1">⚠️ 無料トライアルについて</p>
          <p className="text-xs text-yellow-700">
            無料トライアルはアカウントにつき1回のみご利用いただけます。すでにトライアルをご利用済みのため、
            今回お申し込みいただいたプランは<span className="font-semibold">登録直後から月額料金が発生</span>します。
          </p>
        </div>
      ) : (
      ...
      <p className="text-xs text-gray-400 mt-8 text-center">
        {hasTrialed
          ? 'クレジットカード情報はこの後のStripe画面で入力します。登録直後から課金が開始されます。'
          : 'クレジットカード情報はこの後のStripe画面で入力します。トライアル期間中は請求されません。'
        }
      </p>
    </div>
  )
